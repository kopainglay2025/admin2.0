...
app.post('/api/broadcast/telegram', basicAuthMiddleware, async (req, res) => {
    ...
    try {
        // Fetch all unique chat IDs that have used the Telegram platform
        const chatsSnapshot = await db.collection(CHAT_COLLECTION)
            .where('platform', '==', 'telegram')
            .get();

        const telegramIds = chatsSnapshot.docs.map(doc => doc.id);
        let successCount = 0;
        let failureCount = 0;

        // Broadcast the message sequentially
        for (const chatId of telegramIds) {
            try {
                await bot.sendMessage(chatId, message);
                successCount++;
            } catch (error) {
                // Log Telegram specific errors (e.g., user blocked the bot)
                console.error(`Failed to send broadcast to ${chatId}: ${error.message}`);
                failureCount++;
            }
        }

        console.log(`Telegram Broadcast: ${successCount} successful, ${failureCount} failed out of ${telegramIds.length}`);
        ...
        res.json({
            ...
            message: `Broadcast initiated to ${telegramIds.length} Telegram users. ${successCount} successful, ${failureCount} failed.`
        });
...
app.post('/api/users', basicAuthMiddleware, async (req, res) => {
    try {
        ...
        // Fetch the created document to include the server-generated timestamp
...
// Serve Frontend
app.get('/', (req, res) => {
    // Note: The HTML file must be in the 'public' directory,
    // or we send the file directly from the current directory if it's there.
    // Assuming 'index.html' is in the root directory for simplicity here.
